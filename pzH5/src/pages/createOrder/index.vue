<template>
  <div>
    <div class="container">
      ...
      <!-- 支付二维码弹窗 -->
      <van-dialog :show-confirm-button="false" :show="showCode">
...
//通过一个响应式的变量接收一下h5Companion接口返回的数据及图片
...
onMounted(async () => {
  ...
  console.log(createInfo, "h5Companion");
});
...
const submit = async () => {
  ...
  const { data: orderRes } = await proxy.$api.creatOrder(form);
